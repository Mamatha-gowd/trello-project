...
import { getBoardListAPI, addListAPI } from "./API";
...
class listContainer extends Component {
  ...
  componentDidMount() {
    getBoardListAPI(this.state.boardId).then((res) => {
  ...
  handleAddList = (e, name) => {
    ...
    addListAPI(name, this.state.boardId).then((res) => {
      let lists = [...this.state.lists, { listId: res.id, listName: res.name }];
      this.setState({ lists });
    });
  };
  render() {
